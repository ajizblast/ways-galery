...
const Register = ({ showLogin, dropRegister }) => {
  ...
  const mutationFunction = async () => {
    ...
    return response.data; // Mengasumsikan Anda ingin mengembalikan beberapa data dari mutasi
  };

  const handleSubmit = useMutation(mutationFunction, {
    onError: (err) => {
      console.log("Register Gagal : ", err);
      setAlertMessage(err.response?.data?.message || "Terjadi kesalahan");
      setAlert(true);
    },
  });
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      ...
      // Sisa logika setelah mutasi berhasil
    } catch (err) {
  ...
  return (
      <>
        <div className="absolute z-[30] -top-14 w-full">
          {alert ? <RegisterAlertFailed message={alertMessage} /> : null}
        </div>
        <section id="register">
          <div className="w-[416px] h-[490px] relative z-10 py-10 px-8 rounded-lg bg-white">
            ...
            <form onSubmit={handleFormSubmit} className="w-full my-5">
              <input
              ...
              <button
                  type="submit"
                  className="w-full rounded text-xl font-bold py-2.5 mt-8 text-white bg-[#2FC4B2] hover:bg-[#1a9b8c]"
              >
                Register
              </button>
            </form>
